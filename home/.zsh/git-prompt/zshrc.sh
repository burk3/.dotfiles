...
function update_current_git_vars() {
  ...
  if [[ "$HOST" == "viktor" && "$PWD" =~ ^/home/burke/mnt/[a-zA-Z]+ ]]; then
    local vmwd=${PWD#/home/burke/mnt/}/
    local vmhost=${vmwd%%/*}
    local vmdir=/SevOneNMS/${vmwd#*/}
    if grep -q $vmhost ~/.ssh/config ; then
      _GIT_STATUS=`ssh -n -A ${vmhost} "[[ -d /root/.zsh/git-prompt ]] && cd ${vmdir} && python /root/.zsh/git-prompt/gitstatus.py"`
    else
      _GIT_STATUS=""
    fi
  else
...
git_super_status() {
  if [ -n "$__CURRENT_GIT_STATUS" ]; then
    STATUS_LENGTH=1
    ((STATUS_LENGTH=$STATUS_LENGTH + ${#GIT_BRANCH}))
    STATUS="$ZSH_THEME_GIT_PROMPT_PREFIX$ZSH_THEME_GIT_PROMPT_BRANCH$GIT_BRANCH%{${reset_color}%}"
    if [ -n "$GIT_REMOTE" ]; then
      ((STATUS_LENGTH=$STATUS_LENGTH + ${#GIT_REMOTE}))
      STATUS="$STATUS$ZSH_THEME_GIT_PROMPT_REMOTE$GIT_REMOTE%{${reset_color}%}"
    fi
    ((STATUS_LENGTH=$STATUS_LENGTH + 1))
    STATUS="$STATUS$ZSH_THEME_GIT_PROMPT_SEPARATOR"
    if [ "$GIT_STAGED" -ne "0" ]; then
      ((STATUS_LENGTH=$STATUS_LENGTH + ${#GIT_STAGED} + 1))
      STATUS="$STATUS$ZSH_THEME_GIT_PROMPT_STAGED$GIT_STAGED%{${reset_color}%}"
    fi
    if [ "$GIT_CONFLICTS" -ne "0" ]; then
      ((STATUS_LENGTH=$STATUS_LENGTH + 1 + ${#GIT_CONFLICTS}))
      STATUS="$STATUS$ZSH_THEME_GIT_PROMPT_CONFLICTS$GIT_CONFLICTS%{${reset_color}%}"
    fi
    if [ "$GIT_CHANGED" -ne "0" ]; then
      ((STATUS_LENGTH=$STATUS_LENGTH + 1 + ${#GIT_CHANGED}))
    ...
    echo "$STATUS_LENGTH $STATUS"
	fi
}

# Default values for the appearance of the prompt. Configure at will.
ZSH_THEME_GIT_PROMPT_PREFIX="%{$fg[yellow]%}["
ZSH_THEME_GIT_PROMPT_SUFFIX="%{$fg[yellow]%}]%{$fg[blue]%}─"
